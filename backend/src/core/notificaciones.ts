    ...
    for (const a of aplicaciones) {
      ...
      if (a.alertaVencimiento) {
        notificaciones.push({
          ...
          detalle: `${a.huerta.nombre} — ${a.grupos.flatMap((g) => g.productos).map((p) => p.producto.nombreComercial).join(" + ")}`,
          urgente: true,
      ...
      if (a.alertaPendienteAplicar) {
        notificaciones.push({
          ...
          detalle: `${a.huerta.nombre} — ${a.grupos.flatMap((g) => g.productos).map((p) => p.producto.nombreComercial).join(" + ")} (${(a.porcentajeAvance ?? 0).toFixed(0)}% avance)`,
          urgente: true,
          fecha: a.fechaCreacion.toISOString(),
          enlace: `/aplicaciones?huertaId=${a.huertaId}&id=${a.id}`,
        });
    ...
    for (const f of granular) {
      ...
      if (f.alertaVencimiento) {
        notificaciones.push({
          ...
          detalle: `${f.huerta.nombre} — ${f.grupos.flatMap((g) => g.productos).map((p) => p.producto.nombreComercial).join(" + ")}`,
          urgente: true,
      ...
      if (f.alertaPendienteAplicar) {
        notificaciones.push({
          ...
          detalle: `${f.huerta.nombre} — ${f.grupos.flatMap((g) => g.productos).map((p) => p.producto.nombreComercial).join(" + ")} (${(f.porcentajeAvance ?? 0).toFixed(0)}% avance)`,
          urgente: true,
          fecha: f.fechaCreacion.toISOString(),
          enlace: `/fertilizantes/granular?huertaId=${f.huertaId}&id=${f.id}`,
        });
