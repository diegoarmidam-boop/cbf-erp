...
export default function Solicitudes() {
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 16 }}>Solicitudes pendientes de autorizar</h2>

      {error && <div className="tag tag-danger" style={{ display: "block", padding: "8px 12px", marginBottom: 12 }}>{error}</div>}

      {solicitudes.length === 0 ? (
        <p style={{ color: "var(--ink-soft)" }}>No hay nada pendiente de tu autorización en este momento.</p>
      ) : (
          ...
          {solicitudes.map((s) => (
            <div key={s.id} className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <div>
                  <span className="tag tag-warning">{ETIQUETAS_TIPO[s.tipo] ?? s.tipo}</span>
